...
export class MCPClient {
  ...
  async connectToServer(serverScriptPath: string) {
    ...
    } else if (serverScriptPath.endsWith('.js') || serverScriptPath.endsWith('.ts')) {
      command = 'node';
    ...
    this.client.setRequestHandler(CreateMessageRequestSchema, async (request) => {
      ...
      try {
        ...
        this.logMessage('debug', 'client', { action: 'sampling-response', textLength: response.text.length });
    ...
    console.log('Tool details:', Object.entries(this.tools).map(([name, tool]) => ({
      name,
  ...
  private createZodSchema(jsonSchema: any): z.ZodType<any> {
    ...
    console.log('    Required fields:', jsonSchema.required?.join(', ') || 'none');
    ...
    try {
      ...
      const response = await generateText({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a helpful Pokédex assistant. When users ask questions or make requests about Pokémon, help them accomplish their goals. Always provide a user-friendly response based on the tool results.',
          },
          {
            role: 'user',
            content: query,
  ...
  private logMessage(level: string, logger: string, data: any) {
